---
title: "Boundaries_Kapa_Meta_Read_position_Only"
author: "Paulius"
date: "2023-04-26"
...
---
title: "Metatable_Boundaries"
output: html_notebook
---
#### Functions
#### a. PrepareTable function
```{r, echo=FALSE}
PrepareTable_NoReadpos<-function(Samplelist=Samplelist,path=path, SNP1pattern=SNP1pattern, SNP2pattern=SNP2pattern){
  library(tidyverse)
  library(data.table)
  table_list<-list()
  element_no=1
  for (i in Samplelist){
    ...
    table1<-table1[!grepl("S", table1$V6),]
    ...
    colnames(table2) <- c('Read_name', "Start", "CIGAR", "Seq", "File", "Barcode_combination") 
    ...
    table3 <- table2 %>% mutate(SNP1="0") %>% mutate(SNP2="0")
...
path<-'/Users/paulius.baltrusis/OneDrive-KarolinskaInstitutet/Different projects/Project 2.Kapa_BoundaryHAP_22/SNPs_all_samples_localBowtie/chr13_bothOrients0425_outputs'
...
Collisions_input<-PrepareTable_NoReadpos(Samplelist = Samplelist, path=path, SNP1pattern = "GATTT", SNP2pattern = "TGGCCC")
```
## OBSOLETE ALTERNATIVE! Filter out reads with any soft clipping whatsoever and store them in another list
```{r}
...
labs(x="Haplotypes", y="Number of Collisions", title = "Haplotypes with distance (d)=-10 to 0")+
  ...
  facet_wrap(.~Sample)+
  theme_bw()
ggsave("CollisionNumber_Distance-10to0_Haps.png", width=10, height=7)
```
...
```
## Plot unfiltered haplotypes, where d=0 (=11th element)
```{r}
master_boundaries_df_final$Dist<-as.numeric(master_boundaries_df_final$Dist)
subset(master_boundaries_df_final, Dist=='11') %>% mutate(Sample= sub("Kapa_S._._","",Sample)) %>%
  ggplot(aes())+
